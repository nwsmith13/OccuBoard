...
export function NewJobsPage() {
  ...
  async function analyzeJob(event) {
    ...
    if (!descriptionQuality.ready) {
      setError("This looks like a short preview, not the full job description. Paste the full posting text before analyzing.");
      descriptionRef.current?.focus();
      return;
    }
      ...
      <Card>
        <form className="grid gap-5" onSubmit={analyzeJob}>
          ...
          <label className="grid gap-2 text-sm font-medium text-ink">
            ...
            {importWarning && <p className="rounded-lg bg-amber-50 px-3 py-2 text-xs font-bold leading-5 text-amber-800 ring-1 ring-amber-100">{importWarning}</p>}
          </label>
...
function assessJobDescriptionQuality(description = "") {
  ...
  const substantialLength = text.length >= 800;
