...
export function BillingLimitModal({ open, title, upgrading = false, onUpgrade, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] grid place-items-center bg-[#07111F]/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="billing-limit-title">
      <section className="w-full max-w-md rounded-2xl bg-white p-5 shadow-soft ring-1 ring-brand-100">
        <div className="flex items-start justify-between gap-3">
          ...
          <button type="button" className="rounded-lg p-2 text-slate-500 hover:bg-brand-50 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-200" onClick={onClose} aria-label="Close upgrade prompt">
            <X size={18} />
          </button>
        </div>
