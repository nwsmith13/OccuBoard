...
export async function exportResumePdf({ content, profile, job, resume, includeWhyThisFits = false, accentColor = "#FF7A00" }) {
...
export async function exportResumeDocx({ content, profile, job, resume, includeWhyThisFits = false, accentColor = "#FF7A00" }) {
...
export function openResumePrintPreview({ content, profile, job, includeWhyThisFits = false, accentColor = "#FF7A00" }) {
  const sections = parseResumeForExport(content, { includeWhyThisFits, profile });
  const fileName = buildResumeFilename({ profile, job, extension: "pdf" });
...
function renderPdf(doc, sections, { accentColor = "#FF7A00" } = {}) {
...
function buildPrintHtml(sections, title, { accentColor = "#FF7A00" } = {}) {
  return `<!doctype html>
<html>
<head>
